...
public class DlgUser extends javax.swing.JDialog {
    ...
    public DlgUser(java.awt.Frame parent, boolean modal) {
        ...
        Object[] row={"ID User","Nama User","Jabatan","Password","[I]ICD 10","[I]Obat Penyakit","[C]Dokter","[A]Jadwal Praktek","[C]Petugas","[L]Pasien","[A]Registrasi","[A]Tindakan Ralan",
                ...
                "[N]Pasien Meninggal Per Bulan","[F]Perbaikan Inventaris","[L]Data Klasifikasi","[I]Harian Klasifikasi","[I]Bulanan Klasifikasi","[D]Sisa Stok 2","[H]Pembayaran Ralan Poli RSUD","[H]Pembayaran Ralan Sentral RSUD"
        };
        ...
        if(TKd.getText().trim().equals("")||TNmUser.getText().trim().equals("")){
            ...
            if(i!= -1){
                Sequel.mengedit("user","id_user=AES_ENCRYPT('"+tbUser.getValueAt(i,0).toString()+"','nur')",
                    ...
                    "data_klasifikasi='"+tbUser.getValueAt(i,664).toString()+"',"+
    ...
    private void tampil() {        
        try{    
            ...
            ps=koneksi.prepareStatement("select AES_DECRYPT(id_user,'nur'),AES_DECRYPT(password,'windi'),"+
                        ...
                        "perbaikan_inventaris,data_klasifikasi,laporan_harian_klasifikasi,laporan_bulanan_klasifikasi,sisa_stok2,pembayaran_ralan_rsud,pembayaran_ralan_rsud2 from user order by AES_DECRYPT(id_user,'nur')");
            try {
                rs=ps.executeQuery();
                while(rs.next()){
                    user="";
                    ...
                    try {
                        if(rs.getString(1).toLowerCase().contains(TCari.getText().toLowerCase())||
                            ...
                            tabMode.addRow(new Object[]{rs.getString(1),
                               ...
                               rs.getBoolean("data_klasifikasi"),
                               rs.getBoolean("laporan_harian_klasifikasi"),
                               rs.getBoolean("laporan_bulanan_klasifikasi"),
                               rs.getBoolean("sisa_stok2"),
